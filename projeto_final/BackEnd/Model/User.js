...
export async function login(email,pw) {
  return openDb().then((db) => {
    return db.get("SELECT * from users where email=? and palavrapasse=?", [email,pw]).then((res) => res);
  });
}

export async function vEmail(email) {
  return openDb().then((db) => {
    return db.get("SELECT * from users where email=?", [email]).then((res) => res);
  });
}
